...
import librosa
import requests
from model import AutoCNN
from pydantic import BaseModel


app = modal.App("auto-cnn-inference")
...
class AudioClassifier:
    ...
    def load_model(self):
        ...
        self.model = AutoCNN(num_classes=len(self.classes))
        ...
        with torch.no_grad():
            ...
            response = {
                ...
                "waveform": {
                    ...
                    "sample_rate": wavefrom_sample_rate,
                    "duration": len(audio_data) / wavefrom_sample_rate
                }
            }
...
def main():
    audio_data, sample_rate = sf.read("audio-tests/chirpingBirds.wav")
    buffer = io.BytesIO()
    ...
    wavefrom_info = result.get("waveform", {})
    if wavefrom_info:
        values = wavefrom_info.get("values", {})
        print(f"Initial 10 values: {[round(v, 4) for v in values[:10]]}...")
        print(f"Duration: {wavefrom_info.get("duration", 0)}")
